...
class Plan(object):
    def __init__(self):
        ...
        self.enemy_boom_picture = pygame.image.load(enemy_boom_picture)
        # 时间标志
    ...
    def create_enemy(self):
        ...
        if self.enemy_dic == 3:
            ...
            if self.enemy_pos[0] < 0:
                self.enemy_dic = 4
        if self.enemy_dead == 0:
            self.screen.blit(self.now_enemy_picture, self.enemy_pos)
        if self.enemy_dead == 1:
            if self.flag == 1:
                self.booming_pos.append(self.enemy_pos)
                self.flag = 0
    ...
    def judge_damage_enemy(self):
        ...
        for bullet_pos in self.my_bullets:
            ...
            if enemy_pos[0] - width / 2 <= bullet_pos[0] <= enemy_pos[0] + width / 2 and enemy_pos[1] <= bullet_pos[1] <= enemy_pos[1] + height:
                ...
                self.enemy_dead = 1
        if self.enemy_dead == 1:
            for booming_pos in self.booming_pos:
                if self.boom_delay < 30:
                    self.screen.blit(self.enemy_boom_picture, booming_pos)
                    self.enemy_dic = 5
                    self.boom_delay += 1
